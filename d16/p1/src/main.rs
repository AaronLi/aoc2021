...
fn parse(bytes: &[u8], start: usize) -> (ParseResult, usize) {
    println!("Parse starting from {}", start);
        ...
        _ => {
            ...
            let total_packet_length = match length_type {
                1 => {
                    ...
                    println!("Operator has {} subpackets", num_subpackets);
                    let mut subpacket_start = start + 18;
                    for _subpacket_num in 0.. num_subpackets {
                        println!("Subpacket {} starting at {}", _subpacket_num, subpacket_start);
...
fn get_version_number_sum(root: ParseResult) -> usize {
    match root {
        ParseResult::LiteralResult(lit) => {
            lit.version as usize  
        },
        ParseResult::OperatorResult(op) => {
            let mut total = op.version as usize;

            for sub_packet in op.parameters {
                total += get_version_number_sum(sub_packet);
            }
...
fn main() {
    ...
    let total_version_number = get_version_number_sum(result.0);

    println!("{}", total_version_number);
}

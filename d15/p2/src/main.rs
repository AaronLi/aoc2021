...
fn main() {
    ...
    for line in input_lines {
        let mut map_row = line.chars().map(|x| x.to_digit(10).expect("Invalid file format")).collect::<Vec<u32>>();
        let single_section_length = map_row.len();
        for x in 0..4{ // repeat 4 additional times
            let mut row_vec = Vec::new(); // the values to add
            let section = single_section_length * x; // grab a section of values of length `single_section_length`
            for section_index in 0..single_section_length { // for every value in the section
                let v = map_row[section + section_index]; // increment and store
                if v == 9 {
                    row_vec.push(1);
                }else{
                    row_vec.push(v + 1);
            ...
            map_row.extend(row_vec);
        }
        map.push(map_row);
    }

    let single_map_height = map.len();

    // copy row 4 times, incrementing its values
    for y in 0..4 {
        let mut rows = Vec::new();
        let section = single_map_height * y;

        for section_index in 0.. single_map_height {
            let mut new_row = Vec::new();

            for value in &map[section + section_index] {
                if value == &9 {
                    new_row.push(1);
                }else{
                    new_row.push(value + 1);
                }
            }
            rows.push(new_row);
        }

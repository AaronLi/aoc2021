...
fn main() {
    ...
    scanner_positions.push(Point3D{x: 0, y: 0, z:0});
                ...
                for z_rotation in [Rotation::_0, Rotation::_90, Rotation::_180, Rotation::_270] {
                    ...
                    for scanner_index in (0..scanners.len()).rev() {
                        ...
                        if points_matching.0 > 11 {
                            //println!("x{} y{} z{} has {} points matching with rotation matrix of\n{}", x_rotation, y_rotation, z_rotation, points_matching.0, points_matching.1);
                            base_scanner.add_points(&points_matching.1, &scanners.remove(scanner_index));
                            scanner_positions.push(Point3D{
                                x: points_matching.1.matrix[0][3],
                                y: points_matching.1.matrix[1][3],
                                z: points_matching.1.matrix[2][3]
                            });
    ...
    for y in 0..scanner_positions.len() {
        for x in y+1..scanner_positions.len() {
            let p1 = scanner_positions[y];
            let p2 = scanner_positions[x];
